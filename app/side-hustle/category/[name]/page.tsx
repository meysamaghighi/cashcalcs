...
export async function generateMetadata({
  ...
  const category = getAllCategories().find((cat) => categorySlug(cat) === name);
  if (!category) return {};
    ...
    <main className="min-h-screen bg-gradient-to-b from-emerald-50 to-white">
      <div className="max-w-4xl mx-auto px-4 pt-8 pb-12">
        ...
        <p className="text-lg text-gray-600 mb-8">
          {hustles.length} {category.toLowerCase()} side hustles ranked by earning potential.
        </p>

        <div className="space-y-4 mb-12">
          {hustles.map((h, i) => (
            <Link key={h.slug} href={`/side-hustle/${h.slug}`} className="block">
              <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow p-5">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${diffColor[h.difficulty]}`}>{h.difficulty}</span>
                  <span className="text-xs font-medium px-2 py-1 rounded-full bg-blue-100 text-blue-800">{h.flexibility} Flexibility</span>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-lg font-bold text-emerald-600">
                    ${h.hourlyLow}-${h.hourlyHigh}/hr
                    <span className="text-sm font-normal text-gray-500 ml-2">
                      (${Math.round(h.hourlyLow * 10 * 4.33).toLocaleString()}-${Math.round(h.hourlyHigh * 10 * 4.33).toLocaleString()}/mo at 10hr/week)
                    </span>
                  </p>
                </div>
              </div>
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Explore Other Categories</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
          {otherCategories.map((cat) => (
            <Link key={cat} href={`/side-hustle/category/${categorySlug(cat)}`} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-4 text-center">
              <p className="font-bold text-gray-900">{cat}</p>
              <p className="text-sm text-gray-500">{sideHustles.filter((h) => h.category === cat).length} hustles</p>
            </Link>
          ))}
        </div>

...
export function generateStaticParams() {
  return loanScenarios.map((s) => ({ slug: s.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/loan-calculator/${slug}`,
    },
  };
}
...
export default async function ScenarioPage({
  ...
  const others = loanScenarios.filter((s) => s.slug !== scenario.slug);

  return (
    <main className="min-h-screen bg-gradient-to-b from-violet-50 to-white">
      <div className="max-w-4xl mx-auto px-4 pt-8 pb-12">
        <nav className="text-sm text-gray-500 mb-6">
          <Link href="/loan-calculator" className="hover:text-violet-600">
            Loan Calculator
          </Link>
          {" > "}
        ...
        <LoanCalculator defaults={scenario.defaults} />
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {others.map((s) => (
              <Link
                ...
                className="bg-gray-50 hover:bg-violet-50 rounded-xl p-4 transition-colors"
              >
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description: scenario.seoDescription,
            url: `https://cashcalcs.com/loan-calculator/${scenario.slug}`,

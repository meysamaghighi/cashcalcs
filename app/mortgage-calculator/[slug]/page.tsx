...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/mortgage-calculator/${slug}`,
    },
  };
}
...
export default async function ScenarioPage({
  ...
  const others = mortgageScenarios.filter((s) => s.slug !== scenario.slug);

  return (
    <main className="min-h-screen bg-gradient-to-b from-sky-50 to-white">
      <div className="max-w-4xl mx-auto px-4 pt-8 pb-12">
        <nav className="text-sm text-gray-500 mb-6">
          <Link href="/mortgage-calculator" className="hover:text-sky-600">
            Mortgage Calculator
          </Link>
          {" > "}
        ...
        <MortgageCalculator defaults={scenario.defaults} />
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {others.map((s) => (
              <Link
                ...
                className="bg-gray-50 hover:bg-sky-50 rounded-xl p-4 transition-colors"
              >
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "WebApplication",
            name: `${scenario.name} Mortgage Calculator`,
            description: scenario.seoDescription,
            url: `https://cashcalcs.com/mortgage-calculator/${scenario.slug}`,
